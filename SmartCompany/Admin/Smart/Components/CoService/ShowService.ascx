<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ShowService.ascx.cs" Inherits="SmartCompany.Admin.Smart.Components.CoService.ShowService" %>
...
<!-- Box Head -->
<div class="box-head">
	<h2 class="title"><asp:Label ID="lbBoxTitle" runat="server" Text="Services Management" meta:resourcekey="lbBoxTitleResource1" /></h2>
	<div class="search">
    ...
    <HeaderTemplate>
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
				...
				<th width="160"><asp:Label ID="lbTitle" runat="server" Text="Title" 
                        meta:resourcekey="lbTitleResource1" /></th>
				<th width="260"><asp:Label ID="lbDescription" runat="server" Text="Description" 
                        meta:resourcekey="lbDescriptionResource2" /></th>
                <th><asp:Label ID="lbUserName" runat="server" Text="User Name" 
                        meta:resourcekey="lbUserNameResource1" /></th>
                <th><asp:Label ID="lbModifiedDate" runat="server" Text="Date" 
                        meta:resourcekey="lbModifiedDateResource1" /></th>
                <th><asp:Label ID="lbDisplay" runat="server" Text="Display" 
                        meta:resourcekey="lbDisplayResource1" /></th>
			...
			<tr>
				...
				<td>
                    <asp:LinkButton ID="lnkBtnDetails" runat="server" OnClick="lnkBtnDetails_Click" 
                        CommandArgument='<%# Eval("Id") %>' Font-Underline="False"><%# Eval("Title") %></asp:LinkButton>                
                </td>
				<td>
                ...
                <td><%# Eval("UserName")%></td>
                <td><%# DataBinder.Eval(Container.DataItem, "ModifiedDate", "{0:dd.MM.yyyy}")%></td>
                <td><%# SmartCompany.Admin.Shared.Code.SharedValues.ShowYesNoValue(Eval("Show").ToString())%></td>
				<td>
                    <asp:LinkButton ID="lnkBtnEdit" runat="server" OnClick="lnkBtnEdit_Click" 
                        CommandArgument='<%# Eval("Id") %>' CommandName='<%# Eval("Title_Id") %>' Text="Edit" CssClass="ico edit" 
